...
export function SupplierListPage() {
  ...
  return (
    <section>
      <h1 className="mb-4 text-2xl font-semibold text-stone-900">Suppliers</h1>
      <ListFilters
        search={list.search}
        ...
        action={
          canAdd ? (
            <Link className="inline-block rounded-md bg-teal-800 px-4 py-2 text-sm font-medium text-white" to="/suppliers/new">
              New supplier
            </Link>
          ) : null
        }
          ...
          columns={[
            {
              ...
              sortKey: "code",
              render: (row) => (
                <Link className="text-teal-800 underline" to={`/suppliers/${row.id}`}>
                  {row.code}
                </Link>
              ),
            },
